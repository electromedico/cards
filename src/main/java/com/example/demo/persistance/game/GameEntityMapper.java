...
public class GameEntityMapper {
  ...
  @Autowired private PlayerEntityMapper playerEntityMapper;

  public Game fromDto(GameEntity gameEntity) {

    List<Player> players =
        gameEntity.getPlayers().stream().map(playerEntityMapper::fromDto).toList();
    ...
    return Game.builder()
        ...
        .deck(deck)
        .players(players)
        .revision(new Revision(gameEntity.getRevision()))
        .build();
  }
  ...
  public GameEntity toDto(Game game) {
    ...
    List<PlayerEntity> playerEntities =
        game.getPlayers().stream().map(playerEntityMapper::toDto).toList();
