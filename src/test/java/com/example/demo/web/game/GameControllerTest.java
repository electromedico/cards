...
class GameControllerTest {
  ...
  @Mock private PlayerJsonMapper playerJsonMapper;
  @Mock private GameJsonMapper gameJsonMapper;
  ...
  public void givenAddDeckRequest_whenAddDeck_thenReturnNewGameId()
    ...
    when(gameService.addDeck(gameId, new Revision(revision)))
  ...
  public void givenAddPlayerRequest_whenAddNewPlayer_thenDelegateToService(
      @Mock AbstractMap.SimpleEntry<Revision, Player> revisionPlayer, @Mock Player player)
      throws PlayerNotFoundException, GameNotFoundFoundException, RevisionsDontMatchException {
    // given
    int gameId = 1;
    int revision = 0;
    int playerId = 0;
  ...
  public void givenRemovePlayerRequest_whenRemoveNewPlayer_thenDelegateToService()
      throws GameNotFoundFoundException, RevisionsDontMatchException, PlayerNotFoundException {
    // given
    int gameId = 1;
    int playerId = 2;
    int revision = 0;
  ...
  public void givenDealCardToPlayerRequest_whenDealCardToPlayer_thenDelegateToService(
      @Mock GameJson gameJson, @Mock Game game)
      throws GameNotFoundFoundException, RevisionsDontMatchException, PlayerNotFoundException,
          EmptyDeckException {
    // given
    int gameId = 1;
    ...
    // when
    GameJson expected =
        gameController.dealCardToPlayer(gameId, playerId, revision, mockHttpServletResponse);
  ...
  @Test
  public void givenGetPlayers_whenGetPlayers_thenDelegateToService(
      @Mock Player player, @Mock PlayerJson playerJson) throws GameNotFoundFoundException {
    // given
    int gameId = 1;
  ...
  public void givenShuffleRequest_whenShuffle_thenDelegateToService()
    ...
    int gameId = 1;
    int revision = 0;
    when(gameService.shuffle(gameId, new Revision(revision)))
        .thenReturn(new Revision(revision + 1));
    // when
